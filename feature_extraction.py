...
softmax_weight = tf.Variable(tf.random_uniform(shape=shape))
softmax_bias = tf.zeros(shape=nb_classes)
softmax_output = tf.matmul(fc7, softmax_weight) + softmax_bias

probs = tf.nn.softmax(softmax_output)
